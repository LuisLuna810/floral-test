...
//   try {
//     const { name, price, description, image,  color, categories, quantity } = req.body;
    
//     if (!name || !price || !description || !image || !color ) {
      
//       return res.status(400).json({ message: 'Todos los campos son requeridos' });
//     }
//     if (isNaN(price)) {
//       return res.status(400).json({ message: 'El precio, el stock y la cantidad deben ser números' });
//     }
//     if (quantity && isNaN(Array.isArray(quantity))) {
//       return res.status(400).json({ message: 'Revisa que el campo sea numerico "Ej: Ramos 50"' });
//     }
    
//     const product = await Product.create({ name, price, description, image, quantity });
...
//       // ESTO CREABA EL STOCK ASOCIADO A UN COLOR AL CREAR EL PRODUCTO
//       } else {
...
const createProduct = async (req, res) => {
  try {
    const { name, description, image, color, categories, price } = req.body;
